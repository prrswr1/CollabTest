function putQuote(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("Quotes"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "putQuote"));
