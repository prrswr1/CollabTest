function insertFile(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("insertdbfile"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "insertFile"));
