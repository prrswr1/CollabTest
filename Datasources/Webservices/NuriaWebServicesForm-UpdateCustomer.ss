function UpdateCustomer(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("NuriaWebServicesForm"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "UpdateCustomer"));
		...
		for(var i=0; i<functionBody.length; i++){
			var paramValue = $s.getParameter(functionBody[i].key);
			body += functionBody[i].key + "=" + ((functionBody[i].allowOverride && paramValue) ? ""+paramValue : functionBody[i].value) + "&";
		}
		body = body.slice(0, -1);
		var response = $s.put(url, body, null, JSON.stringify(headers));
		$s.setStatus(200);
		$s.setData(response);
	} catch(e){
