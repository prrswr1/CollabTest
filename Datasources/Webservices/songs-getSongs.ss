function getSongs(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("songs"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "getSongs"));
